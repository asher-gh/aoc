...
- We use Dijkstra's shortest path finding algorithm to find the shortest path
  based on a weight (in this case is the min heat loss). This is similar to BFS
  path finding used in earlier problems, but uses a priority queue instead of a
  regular queue.
- The benefit of this algorithm is that the first optimal state is usually the
  most optimal state and we find the optimal path much quicker.
